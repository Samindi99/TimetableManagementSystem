...
namespace TimeTableM
{
    public partial class SessionDash : Form
    {
        ...
        private void btnViewCon_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string qu = "SELECT `firstLec` as FIRSTLECTURE, `secondLec` as SECONDLECTURE, `tag` as TAG, `tagType` as TAGTYPE, `grp` as STUDENTGROUP, `subject` as SUBJECT, `noOfStd` as NOOFSTUDENT, `day` as DAYS, `time` as TIMES, `AllDetails` as ALLDETAILS FROM `addsesstion`";
        ...
        private void btnPereral_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string qu = "SELECT `department` as DEPARTMENT,`dep2` as DEPARTMENT, `OfferdYear` as YEAR, `OfferedSem` as SEMESTER, `SubjectName` as SUBJECTNAME, `SubjectCode` as SUBJECTCODE, `NoOfLecHours` as LECTUREHOURS, `NoOfTuHours` as TUTORIALHOURS, `NoOfLabHours` as LABHOURS, `NoOfEveHours` as EVELOTIONHOURS FROM addsubject WHERE dep2 != '' AND department = '" + cmbDepartment.Text + "';";
                MySqlCommand cm = new MySqlCommand(qu, con);
                MySqlDataAdapter da = new MySqlDataAdapter(cm);
        ...
        private void btnView_Click(object sender, EventArgs e)
        {
            ...
            try
            {
                string qu = "SELECT `firstLec` as FIRSTLECTURE, `secondLec` as SECONDLECTURE, `tag` as TAG, `tagType` as TAGTYPE, `grp` as STUDENTGROUP, `subject` as SUBJECT, `noOfStd` as NOOFSTUDENT, `day` as DAYS,`room` as ROOM, `time` as TIMES, `AllDetails` as ALLDETAILS FROM `addsesstion` WHERE day = '" + cmbDayLoc.Text + "' AND time = '" + timee + "'";
                MySqlCommand cm = new MySqlCommand(qu, con);
                MySqlDataAdapter da = new MySqlDataAdapter(cm);
